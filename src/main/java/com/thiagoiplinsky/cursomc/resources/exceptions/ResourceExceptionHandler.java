...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> validation(MethodArgumentNotValidException e, HttpServletRequest request) {
		...
		// Erro personalizado para validação                                               		COD. 422
		...
		for (FieldError x : e.getBindingResult().getFieldErrors()) {
			err.addError(x.getField(), x.getDefaultMessage()); // __x.getField captura o nome do erro e o
																// __x.getDefaultMessage captura a mensagem de erro
		}
	...
	public ResponseEntity<StandardError> authorization(AuthorizationException e, HttpServletRequest request) {
		...
		ValidationError err = new ValidationError(System.currentTimeMillis(), HttpStatus.FORBIDDEN.value(), "Acesso negado",
				e.getMessage(), request.getRequestURI());
	...
	public ResponseEntity<StandardError> file(FileException e, HttpServletRequest request) {
		...
		ValidationError err = new ValidationError(System.currentTimeMillis(), HttpStatus.BAD_REQUEST.value(), "Erro de arquivo",
				e.getMessage(), request.getRequestURI());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(err);
	}
...
//		Captura o status de erro da requisição e atribui ao objeto HttpStatus declarado
